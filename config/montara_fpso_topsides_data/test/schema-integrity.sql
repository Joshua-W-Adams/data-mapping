...
CREATE TABLE ERROR_CODE_FACILITY
(	/*FIELD NAME*/                          /*DATA TYPE AND RESTRICTIONS*/
	`FACILITY_ID`                           VARCHAR(30) NOT NULL,
  ...
  `RGBA`                                  VARCHAR(15),
	...
	/*PRIMARY AND FOREIGN KEY DEFINITIONS*/
	PRIMARY KEY (FACILITY_ID, CODE_ID),
  FOREIGN KEY (CODE_ID) REFERENCES SERVER.ERROR_CODE(CODE_ID) ON UPDATE CASCADE,
  FOREIGN KEY (FACILITY_ID) REFERENCES SERVER.FACILITIES(FACILITY_ID) ON UPDATE CASCADE
...
CALL SERVER.CREATE_REV_HISTORY_TABLE('INTEGRITY_TEST', 'ERROR_CODE_FACILITY', 'FACILITY_ID, CODE_ID, REVISION', NULL, NULL);
...
CREATE TRIGGER ERROR_CODE_FACILITY_REV_HISTORY_AI
AFTER INSERT ON ERROR_CODE_FACILITY
FOR EACH ROW
	BEGIN
		IF @DISABLE_TRIGGERS IS NULL THEN
			INSERT INTO ERROR_CODE_FACILITY_REV_HISTORY 	(
													SELECT 'INSERT', IFNULL((SELECT MAX(REVISION) FROM ERROR_CODE_FACILITY_REV_HISTORY WHERE FACILITY_ID = NEW.FACILITY_ID AND CODE_ID = NEW.CODE_ID), 0) + 1, NOW(), IFNULL(@CURRENT_SESSION_USER, USER()), D.*
													FROM ERROR_CODE_FACILITY AS D
													WHERE D.FACILITY_ID = NEW.FACILITY_ID AND D.CODE_ID = NEW.CODE_ID
													);
		END IF;
	END$$

CREATE TRIGGER ERROR_CODE_FACILITY_REV_HISTORY_AU
AFTER UPDATE ON ERROR_CODE_FACILITY
FOR EACH ROW
	BEGIN
		IF @DISABLE_TRIGGERS IS NULL THEN
			INSERT INTO ERROR_CODE_FACILITY_REV_HISTORY 	(
													SELECT 'UPDATE', IFNULL((SELECT MAX(REVISION) FROM ERROR_CODE_FACILITY_REV_HISTORY WHERE FACILITY_ID = NEW.FACILITY_ID AND CODE_ID = NEW.CODE_ID), 0) + 1, NOW(), IFNULL(@CURRENT_SESSION_USER, USER()), D.*
													FROM ERROR_CODE_FACILITY AS D
													WHERE D.FACILITY_ID = NEW.FACILITY_ID AND D.CODE_ID = NEW.CODE_ID
													);
		END IF;
...
CREATE TRIGGER ERROR_CODE_FACILITY_REV_HISTORY_BD
BEFORE DELETE ON ERROR_CODE_FACILITY
FOR EACH ROW
	BEGIN
		IF @DISABLE_TRIGGERS IS NULL THEN
			INSERT INTO ERROR_CODE_FACILITY_REV_HISTORY 	(
													SELECT 'DELETE', IFNULL((SELECT MAX(REVISION) FROM ERROR_CODE_FACILITY_REV_HISTORY WHERE FACILITY_ID = OLD.FACILITY_ID AND CODE_ID = OLD.CODE_ID), 0) + 1, NOW(), IFNULL(@CURRENT_SESSION_USER, USER()), D.*
													FROM ERROR_CODE_FACILITY AS D
													WHERE D.FACILITY_ID = OLD.FACILITY_ID AND D.CODE_ID = OLD.CODE_ID
													);
		END IF;
	END$$
